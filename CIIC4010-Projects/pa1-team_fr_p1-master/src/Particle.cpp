...
void Particle::update(){
	...
	else if(mode  == PARTICLE_MODE_UMBRELLAFORSNOW){ //Recreate the Snow mode but with a Umbrella Effect
        repelFromPoint(ofGetMouseX(), ofGetMouseY());
...
void Particle::draw(){
	...
	if( mode == PARTICLE_MODE_ATTRACT ){
		ofSetColor(takeColor(), takeColor(), takeColor());
	...
	else if( mode == PARTICLE_MODE_UMBRELLAFORSNOW ){
...
//------------------------------------------------------------------
int Particle::takeColor() {		// <---------- Method that change the values to random in colors
	int take_Color = ofRandom(0,255);
	return take_Color;	
}
